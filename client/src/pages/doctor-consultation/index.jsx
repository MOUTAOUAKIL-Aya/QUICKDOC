...
const DoctorConsultation = () => {
  ...
  // OPTION 1: Si BookingModal.jsx crée déjà le rendez-vous
  const handleConfirmBooking = async (bookingResult) => {
    try {
      console.log('Booking result received:', bookingResult);
      
      // Vérifier si la réservation a réussi
      // bookingResult devrait contenir { success: true, appointment: {...}, doctor: {...} }
      if (bookingResult && bookingResult.success !== false) {
        setShowBookingModal(false);
        setBookingSuccess(true);
        setTimeout(() => setBookingSuccess(false), 5000);
        
        console.log('Appointment created successfully:', bookingResult.appointment);
      } else {
        // Le modal a déjà montré l'erreur, on ne fait rien ici
        console.log('Booking was not successful');
      }
      
    } catch (error) {
      console.error('Error in booking confirmation callback:', error);
      // Ne pas montrer d'alerte ici car elle a déjà été montrée dans le modal
    }
  };

  // OPTION 2: Si BookingModal.jsx NE crée PAS le rendez-vous (alternative)
